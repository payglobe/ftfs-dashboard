<?php
/**
 * FTFS Dashboard v2.0 - Receipt Generator (DomPDF)
 *
 * Descrizione:
 * Generatore di scontrini PDF tramite libreria DomPDF. Crea un documento PDF
 * personalizzato a partire da HTML/CSS con i dati della transazione FTFS.
 * Utilizzato come fallback o alternativa a scontrino_nets.php.
 *
 ...
 *
 * Dipendenze:
 * - DomPDF 3.1 (composer require dompdf/dompdf)
 * - Autoloader Composer: ../vendor/autoload.php
 *
 * Flusso Generazione:
 * 1. Recupera Trid da parametro GET
 * 2. Query database per dati transazione + store (JOIN)
 * 3. Genera HTML dello scontrino con dati
 * 4. Converte HTML in PDF tramite DomPDF
 * 5. Output PDF con header Content-Type: application/pdf
 *
 * Database Query:
 * - Tabella principale: ftfs_transactions (alias: ft)
 * - Join: stores (alias: st) ON TerminalID
 * - Campi stores: Insegna, Ragione_Sociale, indirizzo, citta, prov, cap
 * - Campi transaction: tutti (ft.*)
 *
 * Dati Scontrino:
 * - Intestazione: Insegna, Ragione Sociale, indirizzo completo
 * - Transazione: Trid, TermId, DtPos, Amount
 * - Pagamento: PosAcq (circuito), Pan (mascherato), ApprNum
 * - Stato: Conf (C=Confermata, E=Storno, etc.)
 * - Merchant: MeId, Acquirer
 *
 * Headers HTTP Output:
 ...
 * - isRemoteEnabled: false (sicurezza)
 * - defaultFont: 'Helvetica'
 * - Paper size: A4, portrait
 *
 * Errori Gestiti:
 * - 400 Bad Request: Trid non fornito
 * - 404 Not Found: Transazione non trovata
 * - 500 Internal Server Error: Errore query database
 *
 * Sicurezza:
 * - Input sanitization (Trid)
 * - Prepared statements (SQL injection safe)
 * - Session validation (commentata ma disponibile)
 * - No remote file inclusion (DomPDF)
 *
 * Utilizzo Tipico:
 * - Fallback se scontrino_nets.php (API esterna) non disponibile
 * - Scontrini personalizzati con branding PayGlobe
 * - Testing locale senza dipendenze API esterne
 *
 * Note:
 * - Vendor path: ../vendor/ (parent directory)
 * - Richiede composer install in /var/www/html/ftfs/
 *
 * @author Claude Code
 * @version 2.0
 * @date Novembre 2025
